...
async function attachExtras(kind, snapshot) {
  ...
  if (changed) await putEntry(entry);
}
...
document.addEventListener('submit', event => {
  ...
  if (kind === 'drink') {
    ...
    if ((photo instanceof File && photo.size) || note) {
      attachExtras('drink', { date:localDate(), time:String(fd.get('time') || ''), photo, note }).catch(console.error);
    }
  }
  if (kind === 'meal') {
    const fd = new FormData(form);
    ...
    if (fd.get('liquidEstimate') === 'custom' && customOz > 0) {
      attachExtras('meal', { date:localDate(), time:String(fd.get('time') || ''), customOz }).catch(console.error);
    }
  }
}, true);
