...
export function buildWorld(scene){
  ...
  const water=new THREE.Mesh(waterGeo,new THREE.MeshPhysicalMaterial({color:'#6e8f82',transparent:true,opacity:.68,roughness:.18,metalness:.04,transmission:.1}));scene.add(water);
  ...
  for(let i=0;i<55;i++){const m=new THREE.Mesh(new THREE.DodecahedronGeometry(.25+random()*1.05,0),rockMat);const x=-21+random()*15,z=-11+random()*14;m.position.set(x,heightAt(x,z)+.28+random()*.28,z);m.scale.set(1+random()*1.2,.55+random()*.7,.7+random());m.rotation.set(random()*2,random()*6,random()*2);m.castShadow=m.receiveShadow=true;scene.add(m)}

  const logMat=new THREE.MeshStandardMaterial({color:'#5c4832',roughness:1});
  for(let i=0;i<7;i++){const x=-4+random()*14,z=-8+random()*11;const m=new THREE.Mesh(new THREE.CylinderGeometry(.22,.35,5+random()*4,10),logMat);m.rotation.z=Math.PI/2;m.rotation.y=random()*.6;m.position.set(x,heightAt(x,z)+.35,z);m.castShadow=true;scene.add(m)}

  const bladeGeo=new THREE.ConeGeometry(.055,.9,3),bladeMat=new THREE.MeshStandardMaterial({color:'#769557',roughness:.95});
  const count=1700,blades=new THREE.InstancedMesh(bladeGeo,bladeMat,count),dummy=new THREE.Object3D();
  for(let i=0;i<count;i++){let x=-40+random()*80,z=-32+random()*64;if((x>18&&z<-10)||Math.abs(z-(8+Math.sin(x*.14)*1.5))<3){i--;continue}dummy.position.set(x,heightAt(x,z)+.3+random()*.18,z);dummy.rotation.y=random()*6;const s=.55+random()*1.35;dummy.scale.set(s,s,s);dummy.updateMatrix();blades.setMatrixAt(i,dummy.matrix)}blades.receiveShadow=true;scene.add(blades);

  const leafMat=new THREE.MeshStandardMaterial({color:'#4f7b45',roughness:.7,side:THREE.DoubleSide});
  for(let p=0;p<7;p++){const cx=24+random()*11,cz=4+random()*19;for(let l=0;l<8;l++){const leaf=new THREE.Mesh(new THREE.SphereGeometry(1.1,10,6),leafMat);leaf.scale.set(.35,1.5,.08);leaf.position.set(cx+Math.sin(l*.8)*1.2,heightAt(cx,cz)+1.1+random(),cz+Math.cos(l*.8)*1.2);leaf.rotation.z=(l-3.5)*.2;leaf.rotation.y=l*.8;leaf.castShadow=true;scene.add(leaf)}}

  const puddleMat=new THREE.MeshPhysicalMaterial({color:'#8ca5a1',transparent:true,opacity:.42,roughness:.1});
  [[-5,-19,3.2],[-11,17,2.5],[13,-2,1.7],[28,-5,2.2]].forEach(([x,z,s])=>{const p=new THREE.Mesh(new THREE.CircleGeometry(s,36),puddleMat);p.rotation.x=-Math.PI/2;p.position.set(x,heightAt(x,z)+.025,z);scene.add(p)});
