...
async function dismissModal(page, preference = {}) {
  ...
  const buttons = page.locator('#modalChoices button:not([disabled])');
  const count = await buttons.count();
  if (!count) throw new Error(`Open modal had no enabled choices: ${eyebrow} / ${title}`);

  if (eyebrow === 'RIVER CROSSING') {
    const ferry = page.getByRole('button', { name: /Take the ferry/i });
    if (await ferry.isEnabled().catch(() => false)) await ferry.click();
  ...
  if (eyebrow === 'LANDMARK') {
    ...
    await page.getByRole('button', { name: /Continue on the trail/i }).click();
...
async function chooseRations(page, name) {
  await page.getByRole('button', { name: 'Change rations' }).click();
...
async function runJourney(browser, seed, { verifyPersistence = false } = {}) {
  ...
  assert(await page.locator('canvas').count() >= 1, 'PixiJS canvas did not initialize');
  ...
  // Prove IndexedDB save survives a page reload and Resume uses it.
  ...
  while (actions < maxActions) {
    ...
    // A simple conservative human-like strategy.
    const lowestSickHp = Math.min(...s.party.filter((m) => m.alive && m.statuses.length).map((m) => m.hp), 101);
  ...
  // Consume a terminal dialog if one appeared after the last action.
